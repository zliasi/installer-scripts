#!/usr/bin/env bash
# Install PMIx (Process Management Interface for Exascale) with version management via symlinks.
#
# Usage: install-pmix.sh [VERSION] [SYMLINK_NAME]
#
# Arguments:
#   VERSION      - PMIx version (default: 4.2.8)
#   SYMLINK_NAME - Name for symlink (default: default)
#
# Paths:
#   Source: ~/software/src/external
#   Build:  ~/software/build/pmix/VERSION

set -euo pipefail

readonly VERSION="${1:-4.2.8}"
readonly SYMLINK_NAME="${2:-default}"
readonly SRC_DIR="${HOME}/software/src/external"
readonly BUILD_DIR="${HOME}/software/build/pmix/${VERSION}"
readonly SYMLINK_DIR="$(dirname "${BUILD_DIR}")"
readonly ARCHIVE="pmix-${VERSION}.tar.gz"
readonly SOURCE_DIR="${SRC_DIR}/pmix-${VERSION}"
...
# Downloads and extracts PMIx source archive.
#
# Exit codes:
...
download_and_extract() {
  if [[ ! -d "${SOURCE_DIR}" ]]; then
    if [[ -f "${SRC_DIR}/${ARCHIVE}" ]]; then
      ...
      echo "Downloading PMIx ${VERSION}..."
      wget -P "${SRC_DIR}" "https://github.com/pmix/pmix/releases/download/v${VERSION}/${ARCHIVE}" || {
...
# Configures, builds, and installs PMIx from source.
#
# Exit codes:
...
# Creates symlink to current PMIx version.
#
# Exit codes:
...
# Verifies PMIx installation.
#
# Exit codes:
#   0 - Installation verified successfully
#   1 - PMIx library not found
verify_installation() {
  [[ -f "${BUILD_DIR}/lib/libpmix.so" ]] || {
